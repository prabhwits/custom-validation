...
async function validateMessageId(
  ...
  try {
    ...
    const pickedMessageId = pickedMessageIdRaw ? JSON.parse(pickedMessageIdRaw) : null;
...
async function validateTransactionId(
  ...
  try {
    ...
    console.error(
      `!!Error while validating transaction id for /${constants.ON_STATUS_OUT_FOR_DELIVERY}, ${error.stack}`
    );
    result.push(
...
async function validateFulfillments(
  ...
  for (const fulfillment of order.fulfillments || []) {
    ...
    const ffStateCheck = ffDesc?.hasOwnProperty('code') && ffDesc.code === 'Out-for-delivery';
    if (!ffStateCheck) {
      result.push(
        createError(
...
const checkOnStatusOutForDelivery = async (
  ...
  try {
    ...
    await Promise.all([
      ...
      validateOutForDeliveryTimestamps(order, context, transaction_id, state, result),
