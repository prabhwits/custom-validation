...
async function validateFulfillments(
  ...
  for (const ff of order.fulfillments || []) {
    ...
    const ffStateCheck = ffDesc?.hasOwnProperty('code') && ffDesc.code === 'Pending';
    if (!ffStateCheck) {
      result.push(
        createError(
          `Fulfillment state should be 'Pending' in /${constants.ON_STATUS}_${state}`,
          ERROR_CODES.INVALID_ORDER_STATE
        )
      );
    }
  ...
  try {
    if (!data || isObjectEmpty(data)) {
      ...
      const onConfirmOrderState = onConfirmOrderStateRaw
        ? JSON.parse(onConfirmOrderStateRaw)
        : null;
      if (onConfirmOrderState === 'Accepted') {
        result.push(
          createError(
